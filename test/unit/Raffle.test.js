...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle Unit Tests", function () {
          ...
          describe("performUpkeep", function () {
              it("it can only run if checkUpkep is true", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
              ...
              it("reverts when checkupkeep is false", async function () {
                  await expect(raffle.performUpkeep([])).to.be.revertedWith(
                      "Raffle__UpkeepNeededFailed"
                  )
              })
              it("changes the raffle state, emits an event, and calls the vrf coordinator", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets the lottery, and sends money", async function () {
                  ...
                  for (let i = 0; i < additionalEntrants + startingAccountIndex; i++) {
